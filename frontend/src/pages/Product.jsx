...
const Product = () => {
  ...
  const { products, currency, addToCart, navigate, setCartItems } =
  ...
  const handleBuyNow = async () => {
    ...
    // Add only this product to cart, then navigate to checkout
    setTimeout(async () => {
          ...
          <div className="flex-1 flex flex-col-reverse sm:flex-row gap-3">
            <div className="flex sm:flex-col gap-3 sm:w-[18%] w-full overflow-auto">
              {Array.from({ length: 4 }, (_, i) => (
                <div
                  key={i}
                  className="w-20 h-20 sm:w-full sm:h-24 bg-gray-200 rounded-md animate-pulse"
                ></div>
              ))}
            </div>
  ...
  return (
    <div className="border-t-2 pt-10 mt-10">
      ...
      <div className="flex gap-12 flex-col sm:flex-row">
        {/* Images */}
        <div className="flex-1 flex flex-col-reverse sm:flex-row gap-3">
          <div className="flex sm:flex-col gap-3 sm:w-[18%] w-full overflow-x-auto sm:overflow-hidden no-scrollbar">
                ...
                <div className="p-4">
                  ...
                  {productData.subCategory === "Topwear" && (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {Object.entries(sizeChartTopwear).map(
                            ([size, info], index) => (
                              <tr
                                key={size}
                  ...
                  {productData.subCategory === "Bottomwear" && (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {Object.entries(sizeChartBottomwear).map(
                            ([size, info], index) => (
                              <tr
                                key={size}
                                className={`border-b border-gray-100 hover:bg-cyan-25 transition-colors ${
                  ...
                  <div className="mt-3 p-3 bg-blue-50 rounded-md border border-blue-200">
                    <p className="text-xs text-blue-700 flex items-start gap-2">
                      ...
                      <span>
                        <strong>How to measure:</strong> Lay the garment flat
                        and measure across. For best fit, compare with a similar
